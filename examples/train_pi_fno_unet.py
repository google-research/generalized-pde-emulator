...
"""
Example script for training a Physics-Informed FNO U-Net model
"""
...
TEST_SCENARIOS = {
    ...
    'diff_kdv': [(-1.5, -14.0, -6.0), (-1.2, -18.0, -8.0), (-2.0, -5.0, -9.0)],
    'diff_ks_cons': [(-1.5, -1.5, -20.0), (-1.0, -1.2, -15.0), (-1.0, -2.0, -30.0)],
    'diff_fisher': [(0.03, -0.03, 1.0), (0.03, -0.03, 5.0), (0.02, -0.02, 7.5)],
    'diff_adv_diff': [(-2.0, 3.0), (1.0, 2.0), (-4.0, 10.0)],
}

# --- Training Setup ---
SEED = 42
NUM_TRAIN_STEPS = 100000
BATCH_SIZE = 64  # Smaller batch size due to higher memory usage with derivatives
WARMUP_STEPS = 5000
...
def main():
    ...
    print("Creating PI-FNO-UNET model...")
    num_spatial_dims = 1
    in_channels = 1
    model = PI_FNO_UNET(
        ...
        key=model_key,
        encoding_mean=encoding_mean, # Pass to model
        encoding_std=encoding_std,   # Pass to model
    )
    ...
    plt.tight_layout()
    plt.savefig('pi_fno_unet_training_loss.png')
    print("Training loss plot saved as 'pi_fno_unet_training_loss.png'")

    # Save model
    os.makedirs('saved_models', exist_ok=True)
    model_path = os.path.join('saved_models', 'pi_fno_unet_model.eqx')
